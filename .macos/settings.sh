...
osascript -e 'tell application "System Settings" to quit' || {
    echo "Failed to close System Settings"
    exit 1
}
...
# Sandboxed apps require writing to their plist via full path
PREFS="$HOME/Library/Preferences"
SAFARI_PLIST="$PREFS/com.apple.Safari"
MAIL_PLIST="$PREFS/com.apple.mail"
FINDER_PLIST="$PREFS/com.apple.finder.plist"
...
# See https://github.com/li-kai/dotfiles/issues/237
...
# Disable captive portal detection (manually visit http://neverssl.com instead)
...
# Charger: screen off 15m, never sleep, lid wakes, auto-restart
# Battery: screen off 2m, sleep 5m, lid wakes, auto-restart
# Standby: after 24h of sleep, write RAM to disk and power it off (default 1h)
sudo pmset -c displaysleep 15 sleep 0 lidwake 1 autorestart 1 standbydelay 86400
sudo pmset -b displaysleep 2 sleep 5 lidwake 1 autorestart 1 standbydelay 86400
...
# Don't show thumbnails when taking screenshots
...
/usr/libexec/PlistBuddy -c "Set DesktopViewSettings:IconViewSettings:labelOnBottom false" "$FINDER_PLIST"
...
#defaults write "$SAFARI_PLIST" com.apple.Safari.ContentPageGroupIdentifier.WebKit2BackspaceKeyNavigationEnabled -bool true

# Hide Safari’s bookmarks bar by default
defaults write "$SAFARI_PLIST" ShowFavoritesBar -bool false

# Hide Safari’s sidebar in Top Sites
defaults write "$SAFARI_PLIST" ShowSidebarInTopSites -bool false

# Disable Safari’s thumbnail cache for History and Top Sites
defaults write "$SAFARI_PLIST" DebugSnapshotsUpdatePolicy -int 2

# Enable Safari’s debug menu
defaults write "$SAFARI_PLIST" IncludeInternalDebugMenu -bool true

# Make Safari’s search banners default to Contains instead of Starts With
defaults write "$SAFARI_PLIST" FindOnPageMatchesWordStartsOnly -bool false

# Remove useless icons from Safari’s bookmarks bar
defaults write "$SAFARI_PLIST" ProxiesInBookmarksBar "()"
...
defaults write "$SAFARI_PLIST" WebContinuousSpellCheckingEnabled -bool true
# Disable auto-correct
defaults write "$SAFARI_PLIST" WebAutomaticSpellingCorrectionEnabled -bool false
...
defaults write "$SAFARI_PLIST" WebKitJavaScriptCanOpenWindowsAutomatically -bool false
defaults write "$SAFARI_PLIST" com.apple.Safari.ContentPageGroupIdentifier.WebKit2JavaScriptCanOpenWindowsAutomatically -bool false

# Disable auto-playing video
#defaults write "$SAFARI_PLIST" WebKitMediaPlaybackAllowsInline -bool false
#defaults write "$PREFS/com.apple.SafariTechnologyPreview" WebKitMediaPlaybackAllowsInline -bool false
#defaults write "$SAFARI_PLIST" com.apple.Safari.ContentPageGroupIdentifier.WebKit2AllowsInlineMediaPlayback -bool false
#defaults write "$PREFS/com.apple.SafariTechnologyPreview" com.apple.Safari.ContentPageGroupIdentifier.WebKit2AllowsInlineMediaPlayback -bool false

# Enable “Do Not Track”
# defaults write "$SAFARI_PLIST" SendDoNotTrackHTTPHeader -bool true
...
defaults write "$MAIL_PLIST" DisableReplyAnimations -bool true
defaults write "$MAIL_PLIST" DisableSendAnimations -bool true
...
# Slim down Spotlight results (Raycast uses the index for file search)
# MENU_EXPRESSION = calculator, MENU_CONVERSION = unit conversion
...
# Exclude directories from Spotlight indexing by adding them to the privacy list
# Uses the Spotlight preferences plist to add exclusion paths
...
for dir in "${SPOTLIGHT_EXCLUSIONS[@]}"; do
	if [ -d "$dir" ]; then
		...
		sudo mdutil -i off "$dir" > /dev/null 2>&1
	fi
done
...
for file in "${files[@]}"; do
	...
	ln -sfv "$file" "$HOME/$relative_path" 2>/dev/null
done
